...
**ALWAYS use the Forge MCP tools to get authoritative scope documentation:**

```
Use forge-knowledge MCP tool:
- search-forge-docs("forge scopes storage:app")
- search-forge-docs("jira service management assets scopes")
- search-forge-docs("permissions manifest structure")
```

**DO NOT** copy scope lists from old documentation or guess scope names.
**DO NOT** use outdated or incorrect scope syntax.

### Official Documentation

- **Permissions Reference:** https://developer.atlassian.com/platform/forge/manifest-reference/permissions/
- **Forge Platform Scopes:** https://developer.atlassian.com/platform/forge/manifest-reference/scopes-forge/
- **JSM Assets Scopes:** https://developer.atlassian.com/platform/forge/manifest-reference/scopes-product-jsm/

### Manifest Syntax

**List Format** (most common - use this for Assets imports):
```yaml
permissions:
  scopes:
    - storage:app
...
**Map Format** (only for offline user impersonation):
```yaml
permissions:
  scopes:
    storage:app: {}
...
❌ **WRONG** - Multiple scopes on one line:
```yaml
scopes:
  - storage:app, read:cmdb-object:jira
```

❌ **WRONG** - Quoted scopes in list format:
```yaml
scopes:
  - "storage:app"
```
...
✅ **CORRECT** - One scope per line with hyphen:
```yaml
scopes:
  - storage:app
  - read:cmdb-object:jira
...
1. **Use `forge lint`** - Automatically detects missing scopes by analyzing code
2. **Use `forge lint --fix`** - Automatically adds missing scopes to manifest.yml
3. **Consult MCP tools** - Get authoritative scope lists from official docs
4. **Check this project's manifest.yml** - Reference the working example

### Deployment After Scope Changes

When you add or modify scopes in manifest.yml:

1. Run `forge lint` to validate
2. Run `forge deploy --non-interactive -e development`
3. Run `forge install --non-interactive --upgrade --site <site> --product jira --environment development`

Skipping the upgrade step means the new scopes won't take effect.
